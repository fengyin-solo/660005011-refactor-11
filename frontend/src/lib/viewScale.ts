/**
 * 2D 等高线、3D 曲面以及未来新增呈现方式共用的可视化底座：
 *  - 同一份数据范围（含统一 padding）→ 同一套归一化与 2D/3D 投影
 *  - 同一套 z → 颜色映射
 *  - 同一套路径折线颜色与起点 / 当前步 / 终点标记样式
 */
...
export type MarkerKey = 'start' | 'current' | 'end'
...
export interface MarkerStyle {
  ...
  /** 2D Canvas 标记半径（像素） */
  radius2d: number
  /** 3D 场景中的基准半径（世界单位，会按可视范围比例缩放） */
  radius3d: number
}

/** 起点 / 当前步 / 终点的统一样式定义，顺序即绘制顺序 */
export const MARKER_STYLES: Record<MarkerKey, MarkerStyle> = {
...
export interface ViewRanges {
  x: AxisRange
  y: AxisRange
  /** z 轴不做 padding（仅用于高度与颜色归一化） */
  z: AxisRange
  /**
   * 数据跨度 / 可视跨度。3D 标记半径按此比例缩放，
   * 保证“标记相对数据的大小”在 2D 与 3D 中观感一致。
   */
  fillRatio: number
}

interface PaddedRange extends AxisRange {
  fillRatio: number
}

function paddedRange(values: number[], padRatio: number): PaddedRange {
...
export function computeViewRanges(path: IterationPoint[]): ViewRanges | null {
...
export interface Projector {
  /** 数据坐标 → 2D 画布像素坐标（y 轴向上） */
  to2D(p: Pick<Point3, 'x' | 'y'>): { px: number; py: number }
  /** 数据坐标 → 3D 世界坐标，返回 [x, z, y]（three.js 的 y 轴为高度） */
  to3D(p: Point3): [number, number, number]
  /** z 归一化到 0..1（低 → 0，高 → 1），颜色映射统一使用 */
  tz(z: number): number
  /** 某标记样式在当前 3D 场景中的有效世界半径 */
  markerRadius3d(style: MarkerStyle): number
}
 ...
 */
export function createProjector(ranges: ViewRanges, width = 1, height = 1): Projector {
  const nx = (v: number) => (v - ranges.x.min) / (ranges.x.span || 1)
  const ny = (v: number) => (v - ranges.y.min) / (ranges.y.span || 1)
  const nz = (v: number) => (v - ranges.z.min) / (ranges.z.span || 1)
  return {
    to2D(p) {
 ...
 */
export function colorRgb(t: number): [number, number, number] {
  return [
    Math.round(255 * t),
...
export function colorCss(t: number, alpha = 1): string {
  const [r, g, b] = colorRgb(t)
  return `rgba(${r},${g},${b},${alpha})`
}

/** three.js 顶点色 0..1（3D Points） */
export function colorThree(t: number): [number, number, number] {
  const [r, g, b] = colorRgb(t)
 ...
 * 的统一摆放规则，所有画面共用。
 */
export function markerPlacements(path: Point3[], animPath: Point3[]): MarkerPlacement[] {
  if (path.length === 0 || animPath.length === 0) return []
  return [
